...
>(({ className = "", ...props }, ref) => (
  <div ref={ref} className={`w-96 font-sans ${className}`} {...props}>
    <div className="grid grid-cols-4 gap-px bg-neutral-200 rounded-2xl overflow-hidden border border-neutral-200 shadow-lg">
      {stats.map(({ value, label }, i) => (
        <div
          key={label}
          className={`bg-white p-4 text-center ${i === 0 ? "rounded-l-2xl" : ""} ${i === stats.length - 1 ? "rounded-r-2xl" : ""}`}
        >
          <p className="text-xl font-light text-neutral-900 tracking-tight">
            {value}
          </p>
    ...
    <div className="flex items-center justify-center gap-6 mt-4">
      {["Product Hunt", "Hacker News", "Dev.to"].map((platform) => (
        <div key={platform} className="flex justify-center items-center gap-1.5">
          <div className="flex gap-px">
            {[1, 2, 3, 4, 5].map((s) => (
              <Star
                key={s}
                className="rounded-full text-amber-400 fill-amber-400"
                size={9}
              />
            ))}
          </div>
