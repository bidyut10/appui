  ...
  return (
    <div className="grid grid-cols-2 gap-3 md:grid-cols-6 md:gap-4">
      ...
      <article className={cn(card, "col-span-1 p-4 md:col-span-1")}>
        ...
        <p className="mt-2 font-sans text-2xl font-semibold text-neutral-900">
          {rate}
        </p>
      </article>
      ...
      <article className={cn(card, "col-span-1 p-4 md:col-span-2")}>
        <div className="flex items-center gap-2">
          <LayoutGrid size={14} className="text-rose-600" aria-hidden />
          <p className="font-sans text-xs text-neutral-500">In catalog</p>
        </div>
        <p className="mt-2 font-sans text-2xl font-semibold tabular-nums text-neutral-900">
      ...
      >
        <h3 className="font-sans text-sm font-medium text-neutral-800">Share</h3>
        <ul className="mt-4 space-y-3">
          {stats.topComponents.slice(0, 6).map((row, i) => (
            <li key={row.slug}>
              <div className="mb-1 flex justify-between gap-2 text-xs">
                <span className="truncate text-neutral-800">{row.slug}</span>
