...
export const AppleSpotlight = () => {
  ...
  return (
    <div className="w-80 font-sans">
      <div
        className={`overflow-hidden rounded-2xl border bg-white/70 shadow-2xl shadow-black/20 backdrop-blur-2xl transition-all ${focused ? "border-neutral-300" : "border-white/60"}`}
      >
        <div className="flex items-center gap-3 border-b border-neutral-100/80 px-4 py-3">
          ...
          <div className="py-2">
            {results
              ...
              .map((r, i) => (
                <div
                  key={r.title}
                  className={`flex cursor-pointer items-center gap-3 px-4 py-2.5 ${i === 0 ? "bg-[#007AFF]/10" : "hover:bg-neutral-50"}`}
                >
                  <span className="text-lg">{r.icon}</span>
                  <div>
                    <p
                      className={`text-[13px] ${i === 0 ? "font-semibold text-[#007AFF]" : "text-neutral-800"}`}
                    >
                      {r.title}
                    </p>
