...
export const DashboardSidebarMini = () => {
  ...
  return (
    <div className="w-64 bg-neutral-950 rounded-2xl p-3 font-sans">
      ...
      <nav className="space-y-0.5">
        {links.map(({ icon: Icon, label, badge }) => (
          ...
          >
            ...
            {label}
            {badge && (
              <span className="ml-auto text-[9px] font-mono bg-green-500 text-white px-1 pt-1 pb-0.5 rounded-full">{badge}</span>
            )}
          </button>
        ))}
      </nav>
      <div className="mt-4 pt-3 border-t border-neutral-800 px-2">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-full bg-linear-to-br from-green-400 to-cyan-400" />
          <div>
