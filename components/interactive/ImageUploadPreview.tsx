      ...
      {preview ? (
        <div className="relative rounded-2xl overflow-hidden border border-neutral-200 shadow-lg group">
          ...
          <div className="p-3 bg-white flex items-center justify-between">
            ...
            <button onClick={() => inputRef.current?.click()} className="text-[10px] font-medium text-neutral-600 hover:underline cursor-pointer">Replace</button>
          </div>
        </div>
      ) : (
        <div onClick={() => inputRef.current?.click()} className="h-44 border-2 border-dashed border-neutral-200 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-neutral-300 hover:bg-neutral-50/30 transition-all">
