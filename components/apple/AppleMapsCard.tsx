...
export type AppleMapsRouteData = Record<
  AppleMapsRouteMode,
  { time: string; distance: string }
>;

export type AppleMapsCardProps = {
  title?: string;
  routeData?: AppleMapsRouteData;
  defaultMode?: AppleMapsRouteMode;
  badgeLabel?: string;
  arrivalTime?: string;
  locationLabel?: string;
  markers?: { label: string; position: string }[];
  onModeChange?: (mode: AppleMapsRouteMode) => void;
  onZoomIn?: () => void;
  onZoomOut?: () => void;
  onNavigate?: () => void;
...
const defaultRouteData: AppleMapsRouteData = {
  ...
  transit: { time: "24 min", distance: "5.1 km" },
};

const defaultMarkers = [
  { label: "Victoria Memorial", position: "top-8 left-12" },
  { label: "Howrah Bridge", position: "right-10 bottom-16" },
];

const modes: { id: AppleMapsRouteMode; label: string; icon: ReactNode }[] = [
...
export const AppleMapsCard = forwardRef<HTMLDivElement, AppleMapsCardProps>(
  (
    ...
    const [mode, setMode] = useState<AppleMapsRouteMode>(defaultMode);
    const activeRoute = routeData[mode];

    const handleModeChange = (next: AppleMapsRouteMode) => {
      setMode(next);
      onModeChange?.(next);
    };
    ...
    return (
      ...
      >
        <div
          data-slot="apple-maps-card-map"
          className="relative h-44 overflow-hidden bg-[#f4f3ef]"
        >
          ...
          {markers.map((marker) => (
            <div
              key={marker.label}
              data-slot="apple-maps-card-marker"
              className={cn(
                "absolute rounded-lg bg-white/90 px-2.5 py-1 shadow-md backdrop-blur-xl",
                marker.position,
              )}
            >
          ...
          <div
            data-slot="apple-maps-card-zoom"
            className="absolute top-3 right-3 flex flex-col gap-2"
          >
            <button
              type="button"
              onClick={onZoomIn}
              className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-xl bg-white/90 shadow-md backdrop-blur-xl"
            >
              <Plus size={12} />
            </button>
            <button
          ...
          <div
            data-slot="apple-maps-card-location"
            className="absolute bottom-3 left-3 flex items-center gap-1.5 rounded-2xl bg-white/90 px-3 py-2 shadow-md backdrop-blur-xl"
          >
        ...
        <div data-slot="apple-maps-card-details" className="p-4">
          <div className="mb-4 flex items-start justify-between">
            ...
            <button
              type="button"
              data-slot="apple-maps-card-navigate"
              onClick={onNavigate}
              className="flex h-11 w-11 cursor-pointer items-center justify-center rounded-full bg-[#007AFF] text-white shadow-lg shadow-blue-300/40"
            >
              <Play size={13} />
            </button>
          </div>
          ...
          <div
            data-slot="apple-maps-card-modes"
          ...
          >
            {modes.map(({ id, label, icon }) => (
              <button
                ...
                className={cn(
                  "flex flex-1 cursor-pointer flex-col items-center gap-1 rounded-xl py-2 transition-all",
                  mode === id
