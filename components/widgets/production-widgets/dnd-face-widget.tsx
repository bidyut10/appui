...
function DndMascot({ on }: DndMascotProps) {
  return (
    <g aria-hidden>
      ...
      {on ? (
        <>
          <path d="M64 122 Q68 118 72 122" stroke="white" strokeWidth="2.2" fill="none" strokeLinecap="round" />
          <path d="M104 122 Q108 118 112 122" stroke="white" strokeWidth="2.2" fill="none" strokeLinecap="round" />
        </>
      ) : (
        <>
...
export const DndFaceWidget = forwardRef<
  HTMLButtonElement,
  DndFaceWidgetProps
>(({ className, label = "Focus", defaultOn = true, onClick, ...props }, ref) => {
